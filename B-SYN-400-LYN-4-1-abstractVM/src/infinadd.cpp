...
std::string operations::infinadd(std::string a, std::string b)
{
    int tmp;
    char retenue = '0';
    std::string result = "";
    ...
    unsigned long add = 0;
    unsigned long before_longest = 0;
    ...
    if (coma == true) {
        ...
        for (unsigned long i = after[1].length(); i > 0; i--) {
            tmp = ((static_cast<int>(after[1][i - 1]) - '0') + (static_cast<int>(after[0][i - 1]) - '0') + (static_cast<int>(retenue) - '0'));
            if (tmp > 9) {
                retenue = '1';
                tmp -= 10;
            }
    ...
    for (unsigned long i = before[before_longest].length(); i > 0; i--) {
        tmp = ((static_cast<int>(before[1][i - 1]) - '0') + (static_cast<int>(before[0][i - 1]) - '0') + (static_cast<int>(retenue) - '0'));
        if (tmp > 9) {
            retenue = '1';
            tmp -= 10;
        } else
